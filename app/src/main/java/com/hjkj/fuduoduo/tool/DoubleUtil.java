...
public class DoubleUtil implements Serializable {
    ...
    /**
     * 若小数点后面为0，保留整数，并去除科学记数法显示
     *
     * @param num
     * @return
     */
    public static String doubleTransf(double num) {
        if (Math.round(num) - num == 0) {
            return double2Str((long) num);
        }
        return double2Str(num);
    }

    public static String double2Str(Number d) {
        if (d == null) {
            return "";
        }
    ...
    /**
     * Double 转string 去除科学记数法显示(double类型转换显示“.00”)
     *
     * @param d
     * @return
     */
    public static String double2Str(String d) {
        double balance = Double.parseDouble(d) / 100;
