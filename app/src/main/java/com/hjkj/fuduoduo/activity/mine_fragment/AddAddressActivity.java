...
/**
 * 添加收货地址页面
 * Author：Created by shihongfei on 2019/10/2 16:00
...
public class AddAddressActivity extends BaseActivity {
        ...
        switch (view.getId()) {
            ...
            case R.id.m_tv_save:   // 保存
                if (textIsEmpty(mCetName)) {
                    Toasty.info(AddAddressActivity.this, "请填写收货人姓名").show();
                    return;
                }
                if (textIsEmpty(mCetPhone)) {
                    Toasty.info(AddAddressActivity.this, "请填写收货人手机号").show();
                    return;
                }
                if (textIsEmpty(mTvWriteArea)) {
                    Toasty.info(AddAddressActivity.this, "请选择所在地区").show();
                    return;
                }
                if (textIsEmpty(mCetAddress)) {
                    Toasty.info(AddAddressActivity.this, "请填写详细地址").show();
                    return;
                }
                requestData();
                break;
    ...
    protected void requestData() {
        String userId = UserManager.getUserId(AddAddressActivity.this);
